...
public class IntNumberListProperty extends NumberListProperty {
    private final List<Long> entries;

    public IntNumberListProperty(ByteBuffer meta, ByteBuffer data) {
        super(meta);
        ...
        for (int i = 0; i < entryCount.intValue(); i++) {
            entries.add(meta.getLong());
        }
        selectedIndex = entries.indexOf(data.getLong());
    }

    @Override
    public int getType() {
        return Type.NUMBER_LIST_INT;
    }

    @Override
    public List<Byte> serializeData() {
        ByteBuffer buffer = ByteBuffer.allocate(Long.BYTES);
        buffer.putLong(entries.get(selectedIndex));
    ...
    public List<String> getStringEntries() {
        return entries.stream().map((it) -> Long.toString(it)).collect(Collectors.toList());
    }
